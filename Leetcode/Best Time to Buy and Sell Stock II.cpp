...
int find(vector<int>& prices,  vector<vector<int>>& dp, int index, int buy) {
    if(index >= sz(prices)) {
        return 0;
    }
    if(dp[buy][index] != -1) {
    ...
    if(buy == 0 ) {
        return dp[buy][index] = max(-prices[index] + find(prices, dp, index+1, 1) , find(prices, dp, index+1, buy));
    } else {
       //option of selling at current index
       return dp[buy][index] = max(prices[index] + find(prices, dp, index + 1, 0), find(prices, dp, index+1, buy));
    }
